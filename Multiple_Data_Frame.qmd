...
If a star does not appear in the planet data frame, this means that this star has no associated habitable planets. We want to compute the distribution of the average number of habitable planets per star. 
Here we are missing the stars without planets. 
...
```{r}
all_stars |>
  summarise(n=sum(!is.na(planet_key)), .by=star_key) |>
  ggplot(aes(x=n)) +
    geom_bar()
```
